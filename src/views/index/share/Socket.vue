			...
			>
				<video
					:class="[
						'remote_video',
						{
							'main-video': self_minor,
							'minor-video': !self_minor,
						},
					]"
					autoplay
					ref="remote_video"
	...
	export default defineComponent({
		...
		setup() {
							...
							await new Promise((resolve, reject) => {
								...
								nextTick(() => {
									...
									const handleAnswer = () => {
										...
										button_answer.removeEventListener(
											'click',
											handleAnswer
										)
									}
									button_answer.addEventListener(
										'click',
										handleAnswer
									)
								})
			...
			socket.on('connect_error', err => {
				if (/Authorization/.test(err.message))
			...
			async function handleNegotiationNeededEvent() {
				try {
					...
					if (
						(peerConnection as RTCPeerConnection).signalingState !=
						'stable'
					) {
						return
					}
					...
					await (peerConnection as RTCPeerConnection).setLocalDescription(
						desc
					)
					log('向远端发送 offer')
					// 通过信令服务器发送给远端
					emitToServer({
						...
						data: {
							...
							offer: (peerConnection as RTCPeerConnection)
								.localDescription,
						},
					})
				} catch (err) {
			...
			async function handleIceCandidateEvent(
			...
			// eslint-disable-next-line no-undef
			async function handleVideoOfferMag(
				...
				try {
					if (
						...
						await Promise.all([
							...
							(peerConnection as RTCPeerConnection).setRemoteDescription(
								desc
							),
						])
						return
						...
						// 设置远程描述
						await (peerConnection as RTCPeerConnection).setRemoteDescription(
							desc
						)
					}
				} catch (err) {
				...
				if (!chat.local_media_stream) {
					try {
						...
						handleUserMediaError(err as Error)
					}
				}
				if (self_video.value) {
				...
				try {
					...
					emitToServer({
						...
						data: {
							...
							sponsor: chat.account as string,
							recipient: chat.remote as string,
							answer: (peerConnection as RTCPeerConnection)
								.localDescription,
						},
					})
				} catch (err) {
			...
			// 设置为远端描述
			// eslint-disable-next-line no-undef
			async function handleVideoAnswerMsg(
			...
			async function handleNewICECandidateMsg(
				...
				log('添加收到的候选人')
				try {
					await (peerConnection as RTCPeerConnection).addIceCandidate(
						_candidate
					)
				} catch (err) {
					log_err(err)
