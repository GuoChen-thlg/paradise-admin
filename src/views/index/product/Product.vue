			...
			<el-table-column class-name="product-media" label="产品图片">
				<template #default="props">
					<img
						...
						loading="lazy"
						:src="
							/*props.row.image.src ||*/ createImage(
								500,
								500,
								props.row.title
							)
					...
					<el-button-group>
						<template v-if="user.login_statu">
							<template v-if="user.id !== props.row.vendor.id">
								<el-button
									@click="handleOpenBuyDialog(props.row)"
									size="small"
								>
									查看
								</el-button>
...
<script lang="ts">
	...
	export default defineComponent({
		...
		setup() {
			 ...
			 */
			const handleToggleToWishList = (product: product | null) => {
				if (!product) return
				if (
					user.wishlist.some(
						p =>
			...
			function handleScroll() {
				lineheight =
					...
					) as HTMLTableRowElement | null)?.offsetHeight || lineheight
				const {
					scrollHeight,
					clientHeight,
					scrollTop,
				...
				if (scrollTop > lineheight * buffer) {
					...
					const start = Math.ceil(
						(scrollTop - buffer * lineheight) / lineheight
					)

					table_style.paddingTop = `${lineheight * start}px`

					table_style.paddingBottom = `${productsList.length *
						lineheight -
						scrollTop -
						(count - buffer) * lineheight}px`
