<template>
	<div class="preview-media__wrapper" :style="{ zIndex: zIndex }">
		<div
...
<script lang="ts">
	...
	export default defineComponent({
		...
		setup(props, { emit }) {
			...
			const toggleMode = () => {
				mode.value =
					mode.value.name === Mode.CONTAIN.name
						? Mode.ORIGINAL
						: Mode.CONTAIN
				reset()
			}
			...
			const hide = () => {
				...
				;(document.querySelector(
					'body'
				) as HTMLBodyElement).style.overflow = bodyStyleOver.value
				emit(CLOSE_EVENT)
			}
			...
			onMounted(() => {
				...
				;(document.querySelector(
					'body'
				) as HTMLBodyElement).style.overflow = 'hidden'
			})
